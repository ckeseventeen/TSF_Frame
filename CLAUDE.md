...
**This applies to every write target, not just `sys.path`** — model checkpoints, the monitoring SQLite DB, and log dirs must all resolve against the project root. A relative path like `"logs/models/..."` silently follows the CWD. Real bug this caused: `run_monthly_controller.py` had `model_path = f"logs/models/{task_id}_best.pkl"` (relative) while `output_dir` was absolute, so running from an IDE (CWD=`pipelines/`) vs the shell (CWD=project root) read/wrote **two different `REQ_01_best.pkl` files** → same input, different forecast, non-reproducible. Note on reproducibility: XGBoost train+predict is deterministic **within one environment** (`random_state=42`, `n_jobs=-1` does *not* add float non-determinism), but **NOT across conda envs with a different xgboost major version** — xgb 2.1.4 vs 3.2.0 on identical data+seed produced different trees → forecast `1116.75` vs `1188.90`. The canonical project env is **`tsf_env` (xgboost 2.1.4, py3.10)**; running the controller from another interpreter (e.g. a system py3.13/xgb3.2) silently overwrites the shared `logs/models/REQ_01_best.pkl` with a different-version model and changes the forecast. When two runs diverge, suspect in order: (1) a path/CWD split, (2) a different xgboost env, (3) differing input data — **not** intra-env randomness. Fixed by anchoring `_PROJECT_ROOT = _HERE.parents[1]` then `_PROJECT_ROOT / 'logs' / ...` for `model_path` / `store` / `log_dir`. When adding a REQ_XX task or any artifact-persisting code, never use a CWD-relative path.
